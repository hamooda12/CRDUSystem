...
function buieldHeadTable(tableName) {
  ...
  columnsName.forEach((e) => {
    let text = ``
    if (e.default) {
      text = ` <input type="${gettype(e.type)}" class="form-control s" name="${e.name}" required placeholder="${e.default}" id="insertData"></div>`
    } else if (e.null === "YES") {
      text = ` <input type="${gettype(e.type)}" class="form-control s" name="${e.name}" required placeholder="You can left it empty" id="insertData"></div>`
    } else {
      text = ` <input type="${gettype(e.type)}" class="form-control s" name="${e.name}" required placeholder="without default value" id="insertData"></div>`
    }
                    ...
                    <div class="card-body">
                        ...
                        <div class="row">
                           ${fields}
                            ...
                            <button type="submit" class="btn btn-primary mt-3" id="InsertIntoTable">Insert ${tableName}</button>
                        </form>
                    </div>
                </div>
                </div>
...
function insertData(tableName, dataInsert) {
...
if(columnsName.find((e) => e.key === "PRI"))
    primarykey = e[columnsName.find((e) => e.key === "PRI").name]
  else
    primarykey=columnsName.map((e)=>e);
...
let insertbtn=document.getElementById("InsertIntoTable");
insertbtn.addEventListener("click",(e)=>{
    let fieldvalue = [];
      checkright = {}
      const cell = makearray(document.querySelectorAll("#insertData"))
      console.log(cell)
        errorfieldindex = []
        cell.forEach((e) => {
...
.then(data => {
    ...
    if (data.success) {
      ...
      location.reload();
        ...
        alert("Data insert error" + data.message);
    }
})
.catch(err => {
          ...
          cel.forEach((e, index) => {
            ...
            e.textContent = fieldvalue[index]
              ...
              alert("success update");
                ...
                console.error("you can't update", data.message);
              }
            })
            .catch(err => {
              alert("there is DataBase error hint: check your forgin key data")
            });
